...
export class ConstrutoraCadastroComponent implements OnInit {
  ...
  salvar() {
    if (this.formulario.invalid) {
      Swal.fire('Cadastro de construtora', 'Não é possível salvar a construtora!<br>Existem campos inválidos', 'error');
    } else {
      ...
      if (!this.construtora.id) {
        this.defaultService.salvar('construtoras', this.construtora).subscribe(response => {
          this.construtora = response as Construtora;
        ...
        this.defaultService.atualizar('construtoras', this.construtora).subscribe(response => {
          this.construtora = response as Construtora;
